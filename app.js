...
async function boot() {
  ...
  function showError(msg, detail) {
    app.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100vh;font-size:14px;color:#ef4444;flex-direction:column;padding:20px;text-align:center;"><div style="font-size:48px;margin-bottom:16px;">⚠️</div><div style="margin-bottom:8px;font-weight:bold;">' + msg + '</div><div style="font-size:11px;color:#94a3b8;margin-bottom:8px;max-width:500px;word-break:break-all;">' + detail + '</div><div style="font-size:10px;color:#cbd5e1;margin-bottom:16px;">DEBUG: supabase=' + (typeof supabase === 'undefined' ? 'undefined' : (supabase === null ? 'null' : 'ok')) + ' | EXPERT_DATA=' + (typeof EXPERT_DATA === 'undefined' ? 'undefined' : 'ok') + ' | currentUser=' + (typeof currentUser === 'undefined' ? 'undefined' : (currentUser === null ? 'null' : 'ok')) + ' | isAdmin=' + (typeof isAdmin === 'undefined' ? 'undefined' : isAdmin) + '</div><button onclick="location.reload()" style="padding:8px 20px;background:#2563EB;color:#fff;border:none;border-radius:6px;cursor:pointer;">重新加载</button></div>';
  }
