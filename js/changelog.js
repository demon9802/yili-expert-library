...
var VERSION_CHANGELOG = [
  { version: 'v5.9.4', date: '2026-08-12', summary: '专家卡评分形式统一：综合/专业/影响三项均为「维度X★」圆角实底徽章，仅颜色区分，位置仍在卡片header右上角。座机号码解析增强：兼容8610国际写法与带前缀数据，统一格式化为区号-号码；复制值改为纯数字，不再携带「办公电话」等前缀。手机端联系方式交互统一：电话/邮箱/微信均走底部操作面板，先提供「复制」再执行拨号/发邮件；企微/微信等应用内邮箱仅保留复制并提示受应用限制。仪表盘分值分布换用金色→蓝色→灰色质感配色，并注明分段规则（4.5-5.0★闭区间，其余左闭右开）。管理后台数据维护横幅改为始终显示观察库汇总。观察库说明精简并明确「延后观察」=当前淘汰截止日顺延6个月。观察库专家卡片重排：姓名+三项分值/入库状态/入库原因置顶，评分编辑在下方，操作按钮在右侧。评分管理页子管理员绿色提示框改为普通文字提示。', module: 'UI/联系方式/观察库/仪表盘/评分管理' },
