...
var VERSION_CHANGELOG = [
  { version: 'v5.9.3', date: '2026-08-11', summary: '专家卡评分重设计：综合分深紫实底徽章+金色星（清晰醒目有质感），专业/影响改高对比蓝/橙描边胶囊。评分规则缺失默认值从3★调整为2★，总览表与后台逐专家表均取消综合得分列，测算案例改为虚构示意（避免与已补录专家实情冲突）。观察库操作流程重构：单专家可连续修改多项子维度（橙色高亮暂存），点「确认调分」统一填一次意见并留痕；「持续评估」升级为有实际意义的「延后观察」（淘汰倒计时顺延6个月）；观察期满自动淘汰（系统默认淘汰，留痕，数据保留可恢复）。新增全局操作审计区（主看全部/子看自己），单专家历次调分入口移至子维度评分区；月报Section⑤观察库操作明细。', module: '评分系统/观察库/月报' },
