...
function scoreEducation(expert: Expert): number {
  ...
  if (!edu || edu === '未公开' || edu.startsWith('未公开')) return MISSING
...
function scoreQualification(text: string): number {
  ...
  // 4★：国家级执业 / 行业权威认证（多重）/ 大厂资深技术专家与讲师体系
  if (/国家级执业|国家.*认证|行业权威|高级.*师|高级.*认证|多重认证|多项认证|资深架构师|技术专家|全球\s*mvp|连续\s*\d+\s*届|高级\s*工程师|高级\s*讲师|授课\s*讲师|认证\s*体系\s*负责人|星级\s*讲师|特聘\s*(专家|导师)|课程\s*导师/i.test(t)) return 4
  // 3★：行业厂商认证 / 单一国家级资格
...
function scoreAchievement(text: string, projectText: string): number {
  ...
  // 4★：显著成果（著作、专利、国家级/省部级项目、版权课、方法论、平台从0到1搭建、体系化课程/团队）
  if (/著作|出版|专著|sci|ei|核心期刊|发明专利|国家级.*项目|省部级.*项目|战略级.*项目|开发了|版权课|教材|方法论|从\s*0\s*到\s*1|0\s*到\s*1|平台.*搭建|整合.*(设计师|资源)|组建.*团队|主导.*(项目|平台)|构建了|执教\s*场次|场次\s*\d+|课程\s*体系|论文\s*\d+|发表\s*\d+\s*篇|practice\s*prize|ism\s*prize/i.test(full)) return 4
  // 3★：丰富实战经验（多年、大型企业项目、头部客户服务、实战专家/导师/搭建落地赋能）
  if (/\d+\s*年|资深|多年|丰富|实战\s*专家|专家|导师|搭建|落地|赋能|特聘/i.test(full) && /讲师|培训|课程|开发|项目|服务|咨询|顾问|头部客户|世界500强|央企|上市公司|大型企业|企业|集团|公司|实战|经验|工作|从业|经历|搭建|落地|赋能/i.test(full)) return 3
  // 2★：一般服务经验 / 有从业年限但缺少具体项目关键词
  if (/讲师|培训|课程|项目|服务|咨询|顾问|企业|集团|公司|实战|经验|工作|从业|经历|年/i.test(full)) return 2
  // 1★：仅有公开演讲/参与级
  return 1
}
...
function scoreHonors(text: string): number {
  ...
  // 4★：国家级荣誉或称号 / 权威媒体与行业大奖
  if (/国家.*荣誉|国家.*奖|全国.*奖|国家级.*称号|享受国务院|特殊津贴|央视|创新奖|年度.*大奖|百度百科|百科收录/i.test(t)) return 4
  // 3★：省部级荣誉 / 知名协会理事 / 行业权威头衔（副院长、全球MVP、政协常委、特聘专家等）
  if (/省部级|省级.*奖|部级.*奖|协会.*理事|学会.*理事|专家库|智库|委员|十大|百强|全球\s*mvp|连续\s*\d+\s*届|政协|常委|副主席|副会长|常务理事|主席|副主任|副所长|副院长|特聘\s*(专家|导师)|课程\s*导师|百度百科/i.test(t)) return 3
  // 2★：地市级 / 行业协会成员
...
function scoreTitleAndManagement(text: string): number {
  ...
  // 4★：教授 / 高级企业管理者 / 资深技术专家 / 副院长级与行业权威头衔
  if (/教授|研究员|高级工程师|高级.*师/i.test(t)) return 4
  if (/ceo|总裁|总经理|董事长|创始人|董事局主席|vp|副总裁|合伙人|c.*o/i.test(t)) return 4
  if (/副院长|副所长|副会长|副主席|副秘书长|联合创始人|首席(?!科学家)|技术专家|资深架构师|系主任|副主任|常务理事|主席|理事(长)?|特聘\s*(专家|导师)|课程\s*导师|导师/i.test(t)) return 4
  // 3★：总监 / 资深专家 / 副教授
  if (/总监|高级经理|资深|高级|副教授|高工/i.test(t)) return 3
  // 2★：经理 / 主管 / 一般工程师 / 基层岗位 / 无职称（只要有履历信息）
  if (/经理|主管|负责人|工程师|讲师|员工|专员|助理|顾问|咨询师|培训师/i.test(t)) return 2
  // 2★：有履历描述但无明确职称关键词，按“一般/基层”计，避免批量压到1★
  return 2
}
...
2. 每个评分项独立 1-5★；信息缺失（未填 / 未公开 / 无法核实）的评分项，默认按 2★ 计，避免粗糙模型批量误判。
3. 专业度评分项：①学历与学术背景、②行业资质与认证、③专业成果与经验。
4. 影响力评分项：④社会荣誉与奖项、⑤职称/管理履历与行业地位。
5. 综合评分 < 3★ 的专家自动进入观察库，不在前端展示。
`.trim()
